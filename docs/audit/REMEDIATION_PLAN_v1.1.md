...
**آخر تحديث:** 2026-08-24 (جلسة 5 — G6 مُعلنة، G7 بانتظار تدقيق مستقل) · **الاختبارات:** 569/569 ✓ · **health:** 200
...
| 🏁 **C1-C4 مغلقة كلها** — الإرسال المكرر مات فعلاً | — | — |
...
|---|---|---|
| **SEC-000A** تدوير Gmail+Telegram | ✅ **منجزة ومُتحقق منها حياً** — revoke→جديد→.env 600→تيليجرام وصل+SMTP auth OK | — |
| MIG-201 | ✅ **منجزة فعلياً** — الأعمدة والفهرس على مخطط الإنتاج | — |
| OUT-202 | ✅ **مُفعّل على الإنتاج** (`claim_mode: atomic` منذ 2026-08-24، health 200) | — |
| OUT-203 idempotency (C2) | ✅ فهرس فريد جزئي + dedupe legacy + تخزين المفتاح بعد النجاح | 3f4c1aa |
| OUT-204 state-matching (C3) | ✅ رتب تنازلية صارمة + pmid مجهول يُصرَح لا يُهمس | 3f4c1aa |
| OUT-205 dead-letter | ✅ تنبيه HIGH بصمة outbox.dead عبر cooldown OBS-102 + سبب ≤200 | 3f4c1aa |
| DB-301 | ✅ D3 فهارس ساخنة في schema.sql (D1/D2 أُغليا سابقاً) | aa53227 |
| WA-302 | ✅ تصنيف أخطاء Graph + Retry-After + تطبيع مركزي + سقف 4096 | 0e42ea3 |
| JOBS-304 | ✅ إلغاء تعاوني ضد الزومبي + متابعات حقيقية + منطقة زمنية/catchup + حراس retention | cbe96bf |
| SRV-401 | ✅ حزم auth كاملة S2/S3/S4 (boot-refusal للأسرار الناقصة) | edf44a6 |
| UI-403 | ✅ عودة الجوال + تثبيت التمرير + حالات فشل الإرسال + ترقيم الأقدم | 56b8c32 |
| LOAD-601 + CHAOS-602 | ✅ تقرير مقيس 3-بروفايلات + مصفوفة فشل كاملة (اكتشفتا ثغرتين حقيقيتين وأُغلقتا) | 2aea015 |
| G1-G6 إعلان رسمي | ✅ [GATES_DECLARATION_2026-08-24.md](./GATES_DECLARATION_2026-08-24.md) — بالأدلة | — |
| REAUD-603 (G7) تدقيق مستقل | ⏳ مفتوح عمداً — مراجع غير منفّذ + قياس هادئ + تحقق opt-in حي | جدولة |
| AI-501→FACT-502→OFFR-503 | 🔒 محجوبة على G7 وفق §29 | بعد G7 |
